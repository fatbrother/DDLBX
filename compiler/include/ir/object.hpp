...
public:
    ...
    virtual Value codeGenValue(CodeGenContext& context) override {
        LOG_DEBUG("NTemplateObjectDeclaration::codeGenValue() should not be called");
        return Value::null();
    }
    llvm::Type* codeGenValue(CodeGenContext& context, std::vector<std::shared_ptr<NType>> templateArgs);
